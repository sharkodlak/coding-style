 ...
 * @author    Pavel Štětina <code@shark.kom.cz>
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Sharkodlak_Sniffs_CodeAnalysis_UnusedFunctionParameterSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for (; $next <= $end; ++$next) {
			...
			if ($code === T_STRING && strncmp('func_get_arg', $token['content'], 12) === 0) {
				return;
			} else if ($code === T_VARIABLE && isset($params[$token['content']]) === true) {
